...
public class ConfigValidator
{
    public static float ValidateInterfaceTransparency()
    {
        ...
        if (configuredTransparency is < 0.1f or > 1.0f)
        {
            DiscJockeyPlugin.LogWarning(
                $"DiscJockeyUIManager<ApplyConfig>: Failed to set interface transparency from config - {configuredTransparency} is not within the accepted range of 0.1 to 1.0!");
            return 0.45f;
        }
    ...
    public static Color ValidateInterfaceColour()
    {
        ...
        DiscJockeyPlugin.LogWarning(
            $"DiscJockeyUIManager<ApplyConfig>: Failed to set interface colour from config - {configuredColour} is not a valid hex colour code!");
